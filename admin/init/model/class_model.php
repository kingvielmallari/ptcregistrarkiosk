...
<?php
    ...
    public function fetchAll_payment(){ 
		...
		public function fetchAll_user() { 
			$sql = "SELECT * FROM tbl_usermanagement WHERE user_id != ?"; // Exclude specific user_id
			$stmt = $this->conn->prepare($sql); 
			$excluded_user_id = 16; // The user_id you want to exclude
			$stmt->bind_param("i", $excluded_user_id); // Bind the parameter as an integer
			$stmt->execute();
			$result = $stmt->get_result();
			$data = array();
			while ($row = $result->fetch_assoc()) {
